...
### Session Update - 2026-07-02 WITA - Workspace Payment Full Amount UI Copy Proof

#### Slice

- Active slice: 0062 follow-up - cash vs transfer workspace payment render wording.
- Status: completed with targeted render proof.
- Production runtime patch: none.
- Finance/payment settlement logic patch: none.

#### FACT

- Edit workspace payment modal exposes `data-backend-payable-rupiah`.
- Existing partially paid edit workspace case exposes backend payable `60000`.
- Backend payment basis is `backend_outstanding_settlement`.
- `Bayar Penuh` now explicitly explains that it uses the system outstanding amount, not the original note total.
- Transfer records that backend amount directly.
- Cash opens the change calculator flow.

#### GAP

- No browser/manual QA was run in this slice.
- No transfer reference/proof upload field is designed in this slice.
- No runtime finance bug was proven in this slice.

#### DECISION

- Keep this as a UI/render clarification only.
- Do not touch settlement, ledger, cash detail, stock, HPP, refund, or reporting runtime logic.
- Treat backend outstanding amount as the financial source of truth.
- Lock the wording with render assertion so future edits do not reintroduce ambiguous "total nota" interpretation.
...
```bash
php artisan test tests/Feature/Note/EditTransactionWorkspacePageFeatureTest.php \
  --filter=test_cashier_edit_workspace_exposes_backend_payable_amount_to_payment_calculator
...
PASS
Bundle kecil payment/render PASS
...
The UI now tells cashier/owner that Bayar Penuh means paying the remaining system outstanding amount.

Transfer and cash are clarified without changing payment calculation.

This closes the focused UI wording/render proof after the cash/transfer ledger proof.
