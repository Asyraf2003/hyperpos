(() => {
  ...
  document.addEventListener("click", (event) => {
    if (!addMenu.contains(event.target) && event.target !== addButton) addMenu.classList.add("d-none");
    const removeButton = event.target.closest("[data-remove-line]");
    if (removeButton) NS.removeRow(removeButton.closest("[data-line-item]"));
  ...
  form?.addEventListener("input", (event) => {
    if (event.target.closest("[data-line-item]") || event.target.name?.startsWith("inline_payment[")) NS.updateSummary();
  });
