(() => {
  ...
  const detailUrl = (id) => cfg.detailBaseUrl.replace('__ID__', id);
  const editUrl = (id) => cfg.editBaseUrl.replace('__ID__', id);
  ...
  const render = ({ rows, meta }) => {
    el.sum.textContent = `Total: ${meta.total} karyawan`;
    ...
    el.pag.innerHTML = Array.from({ length: meta.last_page }, (_, i) => i + 1).map((page) => `
      <button type="button" class="btn btn-sm ${page === meta.page ? 'btn-primary' : 'btn-light'} me-1" data-page="${page}">${page}</button>
    `).join('');
    ...
    if (active) {
      active.textContent = meta.sort_dir === 'asc' ? '↑' : '↓';
    }
  };
