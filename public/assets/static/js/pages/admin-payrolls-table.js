  ...
  const renderRows = (rows, m) => {
    ...
    body.innerHTML = rows.map((r, i) => {
      const notesHtml = r.is_reversed && r.reversal_reason
        ? `${esc(r.notes ?? '-')}<div class="small text-danger mt-1">Reversal: ${esc(r.reversal_reason)}</div>`
        : esc(r.notes ?? '-');

      const statusHtml = r.is_reversed
        ? `<div class="small text-danger mt-1">Direversal${r.reversal_created_at ? ` • ${esc(r.reversal_created_at)}` : ''}</div>`
        ...
        ? `
          <div class="d-flex flex-column align-items-center gap-1">
            ...
            <span class="badge ${r.is_reversed ? 'bg-light-danger text-danger' : 'bg-light-success text-success'}">
              ${r.is_reversed ? 'Direversal' : 'Aktif'}
            </span>
          </div>
        `
