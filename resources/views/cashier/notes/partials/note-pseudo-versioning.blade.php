<div class="card">
  ...
  <div class="card-body">
    @php($versioning = $note['pseudo_versioning'] ?? [])
    @php($current = $versioning['current'] ?? [])
    @php($baseline = $versioning['baseline'] ?? null)
    @php($timeline = $versioning['timeline'] ?? [])
      ...
      <div class="col-12 col-xl-5">
        <div class="border rounded p-3 h-100">
          ...
          <div class="ui-key-value d-flex justify-content-between py-2 border-bottom">
            ...
            <div class="text-end text-uppercase">{{ $current['note_state'] ?? '-' }}</div>
          </div>
          <div class="ui-key-value d-flex justify-content-between py-2 border-bottom">
            <small>Ringkasan Line</small>
            <div class="text-end">{{ $current['line_summary_label'] ?? '-' }}</div>
          </div>
          <div class="ui-key-value d-flex justify-content-between py-2 border-bottom">
          ...
          <div class="ui-key-value d-flex justify-content-between py-2 border-bottom">
            ...
            <div class="text-end">{{ number_format((int) ($current['net_paid_rupiah'] ?? 0), 0, ',', '.') }}</div>
          </div>
          <div class="ui-key-value d-flex justify-content-between py-2 border-bottom">
            <small>Total Refund</small>
            <div class="text-end">{{ number_format((int) ($current['total_refunded_rupiah'] ?? 0), 0, ',', '.') }}</div>
          </div>
          <div class="ui-key-value d-flex justify-content-between py-2">
            <small>Sisa Tagihan</small>
            <div class="text-end">{{ number_format((int) ($current['outstanding_rupiah'] ?? 0), 0, ',', '.') }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="col-12 col-xl-7">
        <div class="border rounded p-3 h-100">
          <div class="d-flex justify-content-between align-items-center gap-2 mb-3">
            ...
            @if ($baseline !== null)
              <span class="badge bg-light-{{ $baseline['badge_tone'] ?? 'info' }} text-{{ $baseline['badge_tone'] ?? 'info' }}">{{ $baseline['badge_label'] ?? 'Pseudo Versioning' }}</span>
            @endif
          </div>
          ...
          @else
            <div class="alert alert-light-info mb-3">{{ $baseline['note'] ?? '' }}</div>
            <div class="ui-key-value d-flex justify-content-between py-2 border-bottom">
              <small>Captured At</small>
              <div class="text-end">{{ $baseline['captured_at'] ?? '-' }}</div>
            </div>
            <div class="ui-key-value d-flex justify-content-between py-2 border-bottom">
              <small>Total Sebelum Mutasi Awal</small>
              <div class="text-end">{{ number_format((int) ($baseline['total_rupiah'] ?? 0), 0, ',', '.') }}</div>
            </div>
            <div class="ui-key-value d-flex justify-content-between py-2 border-bottom">
              <small>Refund Wajib Saat Itu</small>
              <div class="text-end">{{ number_format((int) ($baseline['refund_required_rupiah'] ?? 0), 0, ',', '.') }}</div>
            </div>
            <div class="ui-key-value d-flex justify-content-between py-2">
              <small>Target Status</small>
              <div class="text-end">{{ $baseline['target_status'] ?? '-' }}</div>
            </div>
          @endif
        </div>
      </div>
    ...
    <div class="border-top mt-4 pt-4">
      <div class="d-flex justify-content-between align-items-center gap-2 mb-3">
        ...
        <span class="badge bg-light text-dark border">{{ count($timeline) }} Event</span>
      </div>
