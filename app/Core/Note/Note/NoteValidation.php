...
trait NoteValidation
{
    ...
    {
        if (trim($id) === '') throw new DomainException('Note id wajib ada.');
        if (trim($name) === '') throw new DomainException('Customer name wajib ada.');
    }
    ...
    private static function assertValidWorkItems(array $items): void
    {
        foreach ($items as $item) {
            if (!$item instanceof WorkItem) throw new DomainException('Work item tidak valid.');
        }
    }
    ...
    {
        $total = Money::zero();
        foreach ($items as $item) $total = $total->add($item->subtotalRupiah());
        return $total;
    }
}
