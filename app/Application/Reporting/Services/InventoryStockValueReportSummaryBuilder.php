...
final class InventoryStockValueReportSummaryBuilder
{
    public function build(array $snapshotRows, array $movementRows): array
    {
        return [
            ...
            'total_inventory_value_by_average_rupiah' => array_sum(array_column($snapshotRows, 'current_inventory_value_by_average_rupiah')),
            'total_rounding_residual_rupiah' => array_sum(array_column($snapshotRows, 'current_rounding_residual_rupiah')),
            'total_ledger_qty_diff' => array_sum(array_column($snapshotRows, 'ledger_qty_diff')),
            'total_ledger_value_diff_rupiah' => array_sum(array_column($snapshotRows, 'ledger_value_diff_rupiah')),
            ...
            'period_supply_in_qty' => array_sum(array_column($movementRows, 'supply_in_qty')),
            'period_sale_out_qty' => array_sum(array_column($movementRows, 'sale_out_qty')),
            'period_refund_reversal_qty' => array_sum(array_column($movementRows, 'refund_reversal_qty')),
            'period_revision_correction_qty' => array_sum(array_column($movementRows, 'revision_correction_qty')),
