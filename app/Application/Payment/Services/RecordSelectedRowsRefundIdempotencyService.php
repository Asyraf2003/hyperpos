...
final class RecordSelectedRowsRefundIdempotencyService
{
    private const OPERATION = 'record_selected_rows_refund';
    ...
    public function replay(array $payload): ?Result
    {
        ...
        if ($record['request_hash'] !== $scope['hash']) {
            return Result::failure('Idempotency key refund sudah dipakai untuk payload berbeda.', [
                'idempotency_key' => ['IDEMPOTENCY_KEY_PAYLOAD_MISMATCH'],
            ]);
        }

        if ($record['status'] !== 'succeeded') {
            return Result::failure('Refund sedang diproses. Coba ulang beberapa saat lagi.', [
                'idempotency_key' => ['IDEMPOTENCY_KEY_PROCESSING'],
            ]);
        }

        return Result::success($record['result_payload']['data'] ?? [], 'Refund selected rows sudah diproses sebelumnya.');
    }
